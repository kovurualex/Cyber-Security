...
declare -A policies=(
  ...
  [7]="COMPLIANCE_STANDARD, ORA_STIG_RECOMMENDATIONS"
  [8]="ORACLE_PREDEFINED, ORA_ACCOUNT_MGMT"
  [9]="ORACLE_PREDEFINED, ORA_DATABASE_PARAMETER"
  [10]="ORACLE_PREDEFINED, ORA_DV_AUDPOL"
  [11]="ORACLE_PREDEFINED, ORA_DV_AUDPOL2"
  [12]="ORACLE_PREDEFINED, ORA_LOGON_FAILURES"
  [13]="ORACLE_PREDEFINED, ORA_RAS_POLICY_MGMT"
  [14]="ORACLE_PREDEFINED, ORA_RAS_SESSION_MGMT"
  [15]="ORACLE_PREDEFINED, ORA_SECURECONFIG"
)
...
echo "Note - COMPLIANCE_STANDARD: ORA_STIG_RECOMMENDATIONS (Applicable to DB version 21c and above)* "
...
Assessment_Schedule_Status="All_Regions_Target_DBs_Assessment_Schedule_Status.csv"
Audit_Trails_Status="All_Regions_Target_DBs_Audit_Trails_Status.csv"
Audit_policies_Status="All_Regions_Target_DBs_Audit_policies_Status.csv"
echo "region,compartment_name,compartment_id,target_db_name,target_db_id,Assessment_name,Assessment_id,schedule,lifecycle-state" > $Assessment_Schedule_Status
...
echo "region,compartment_name,compartment_id,target_db_name,target_db_id,AuditPolicy_name,AuditPolicy_id,audit-policy-category,audit-policy-name,status" > $Audit_policies_Status
...
for region_csv_file in *_sorted_$csv_file; do
  ...
  while IFS=',' read -r region compartment_name compartment_id target_db_name target_db_id; do
        ...
        # Step 4e: Retrive the assessment schedule status details in csv file
            SA_DETAILS=$(oci data-safe security-assessment get --security-assessment-id "$security_assessment_id" | jq -r '[.data."display-name", .data.id, .data.schedule, .data."lifecycle-state"] | @csv' | sed 's/"//g')  
            UA_DETAILS=$(oci data-safe user-assessment get --user-assessment-id "$user_assessment_id" | jq -r '[.data."display-name", .data.id, .data.schedule, .data."lifecycle-state"] | @csv' | sed 's/"//g')
            echo "$region,$compartment_name,$compartment_id,$target_db_name,$target_db_id,$SA_DETAILS" >> $Assessment_Schedule_Status
            echo "$region,$compartment_name,$compartment_id,$target_db_name,$target_db_id,$UA_DETAILS" >> $Assessment_Schedule_Status
            echo "Generated schedule status csv files successfully"
        ...
        # Step 4g: Retrieve the Audit Policies Enable_Status details in csv file
            sleep 2
            ./"$generated_script2" > "$log_file2" 2>&1
            echo "Retrieved the Audit policies details successfully"
            while IFS=',' read -r display_name id lifecycle_state; do
                if [ "$lifecycle_state" = "ACTIVE" ]; then
                echo "Provisioning the Audit Policies for $region, $target_db_name "
                oci data-safe audit-policy get --audit-policy-id "$id" | jq -r '.data."audit-specifications"[] | [.["audit-policy-category"], .["audit-policy-name"], .["enable-status"]] | @csv' |sed 's/"//g' | sort -t',' -k1,1 -k2,2 -k3,3 > Audit_policies_Details.txt
                    # Read each row from the temporary file and append to the main CSV
                    while IFS=',' read -r audit_policy_category audit_policy_name enable_status; do
                        echo "$region,$compartment_name,$compartment_id,$target_db_name,$target_db_id,$display_name,$id,$audit_policy_category,$audit_policy_name,$enable_status" >> "$Audit_policies_Status"
                    done < Audit_policies_Details.txt  
                fi
            done < "$audit_policy_file"
...
echo "Script execution completed and generated the below status details files"
echo "All_Regions_Target_DBs_Assessment_Schedule_Status.csv"
echo "All_Regions_Target_DBs_Audit_Trails_Status.csv"
echo "All_Regions_Target_DBs_Audit_policies_Status.csv"
