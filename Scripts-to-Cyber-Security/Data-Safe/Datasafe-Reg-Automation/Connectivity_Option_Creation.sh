...
process_sort_csv() {
    ...
    for region in $regions; do
        local region_csv_file="${region}_sorted_ds_co_${csv_file}"
...
if [[ "$choice" -eq 1 ]]; then
            ...
            if [[ -f "All_Regions_Cloud_BaseDatabases.csv" ]]; then
                while IFS=',' read -r region compartment_name compartment_id db_name db_id pdb_name pdbid db_system_id domain listener_port subnet_id db_version vcn_name vcn_id data_safe_status; do
                    if ! grep -q ",$subnet_id$" All_Regions_Private_Endpoints.csv; then
                        ...
                        # Extract the subnet_name corresponding to the subnet_id from All_Regions_VCNs_Subnets.csv
                        subnetname=$(awk -F',' -v id="$subnet_id" '$7 == id {print $4}' All_Regions_VCNs_Subnets.csv)
                        echo "$region,$compartment_name,$pe_display_name,$vcn_name,$subnetname,${compartment_id},${vcn_id},${subnet_id}" >> "$output_file"
                    fi
    ...
    echo "sample file All_region_Create_PE.csv : headers - region,compartment_name,pe_name,vcn_name,subnet_name,compartment_id,vcn_id,subnet_id"
    #removing duplicates from All_region_Create_PE.csv
    awk -F',' '!seen[$0]++ || NR==1' All_region_Create_PE.csv > All_region_Create_PE_no_duplicates.csv
    mv All_region_Create_PE_no_duplicates.csv All_region_Create_PE.csv
    read -p "Enter the file path for All_region_Create_PE.csv - Please update all fields as necessary: " private_endpoints_file
